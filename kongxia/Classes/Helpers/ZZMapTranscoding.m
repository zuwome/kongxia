...
//  zuwome
//
//  Created by 潘杨 on 2017/12/21.
//  Copyright © 2017年 TimoreYu. All rights reserved.
//

#import "ZZMapTranscoding.h"
...
@implementation ZZMapTranscoding
...
- (id)transformFromGPSToGD {
    CLLocationCoordinate2D coor = [ZZMapTranscoding transformFromWGSToGCJ:CLLocationCoordinate2DMake(self.latitude, self.longitude)];
    return [[ZZMapTranscoding alloc] initWithLatitude:coor.latitude andLongitude:coor.longitude];
}

- (id)transformFromGDToBD {
    CLLocationCoordinate2D coor = [ZZMapTranscoding transformFromGCJToBaidu:CLLocationCoordinate2DMake(self.latitude, self.longitude)];
    return [[ZZMapTranscoding alloc] initWithLatitude:coor.latitude andLongitude:coor.longitude];
}

- (id)transformFromBDToGD {
    CLLocationCoordinate2D coor = [ZZMapTranscoding transformFromBaiduToGCJ:CLLocationCoordinate2DMake(self.latitude, self.longitude)];
    return [[ZZMapTranscoding alloc] initWithLatitude:coor.latitude andLongitude:coor.longitude];
}

- (id)transformFromGDToGPS {
    CLLocationCoordinate2D coor = [ZZMapTranscoding transformFromGCJToWGS:CLLocationCoordinate2DMake(self.latitude, self.longitude)];
    return [[ZZMapTranscoding alloc] initWithLatitude:coor.latitude andLongitude:coor.longitude];
}

- (id)transformFromBDToGPS {
    //先把百度转化为高德
    CLLocationCoordinate2D start_coor = [ZZMapTranscoding transformFromBaiduToGCJ:CLLocationCoordinate2DMake(self.latitude, self.longitude)];
    CLLocationCoordinate2D end_coor = [ZZMapTranscoding transformFromGCJToWGS:CLLocationCoordinate2DMake(start_coor.latitude, start_coor.longitude)];
    return [[ZZMapTranscoding alloc] initWithLatitude:end_coor.latitude andLongitude:end_coor.longitude];
}
