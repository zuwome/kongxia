...
@implementation ZZRecordManager
...
{
    NSError *error = nil;
    if (![[ZZRecordManager shareManager] canRecord]) {
...
- (NSString *)recorderMainPath {
    return [ZZFileHelper createPathWithChildPath:kChildPath];
}
...
- (void)startPlayRecorder:(NSData *)recorderData {
    ...
    self.player = [[AVAudioPlayer alloc] initWithData:recorderData error:nil];
...
-(void)sensorStateChange:(NSNotificationCenter *)notification; {
    ...
    if ([[UIDevice currentDevice] proximityState] == YES) {
        ...
        if (!self.player) {//没有播放了，也没有在黑屏状态下，就可以把距离传感器关了
