...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount delegate:(id<TZImagePickerControllerDelegate>)delegate {
    return [self initWithMaxImagesCount:maxImagesCount columnNumber:4 delegate:delegate pushPhotoPickerVc:YES isFromChat:NO];
}

- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<TZImagePickerControllerDelegate>)delegate isFromChat:(BOOL)isFromChat {
    return [self initWithMaxImagesCount:maxImagesCount columnNumber:columnNumber delegate:delegate pushPhotoPickerVc:YES isFromChat:isFromChat];
}

- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<TZImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc isFromChat:(BOOL)isFromChat chatShouldPay:(BOOL)chatShouldPay {
    _pushPhotoPickerVc = pushPhotoPickerVc;
    TZPhotoPickerController *photoPickerVc = nil;
    _isFromChat = isFromChat;
    _chatShouldPay = chatShouldPay;
    if (isFromChat) {
        photoPickerVc = [[TZPhotoPickerController alloc] init];
        photoPickerVc.isFirstAppear = YES;
        photoPickerVc.columnNumber = self.columnNumber;
        photoPickerVc.isFromChat = isFromChat;
        photoPickerVc.chatShouldPay = chatShouldPay;
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<TZImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc isFromChat:(BOOL)isFromChat {
    return [self initWithMaxImagesCount:maxImagesCount columnNumber:columnNumber delegate:delegate pushPhotoPickerVc:pushPhotoPickerVc isFromChat:isFromChat chatShouldPay:NO];
}
...
- (void)observeAuthrizationStatusChange {
    ...
    if ([[TZImageManager manager] authorizationStatusAuthorized]) {
        ...
        if (_isFromChat) {
            TZPhotoPickerController *photosPickerVc = (TZPhotoPickerController *)self.visibleViewController;
            [[TZImageManager manager] getCameraRollAlbum:self.allowPickingVideo allowPickingImage:self.allowPickingImage needFetchAssets:NO completion:^(TZAlbumModel *model) {
                photosPickerVc.model = model;
                [photosPickerVc fetchAssetModels:NO];
            }];
        }
        else {
            [self pushPhotoPickerVc];
...
- (void)viewDidLoad {
    ...
    cancelItem.tintColor = UIColor.greenColor;
...
+ (void)configBarButtonItem:(UIBarButtonItem *)item tzImagePickerVc:(TZImagePickerController *)tzImagePickerVc {
    item.tintColor = UIColor.blackColor;//tzImagePickerVc.barItemTextColor;
    NSMutableDictionary *textAttrs = [NSMutableDictionary dictionary];
    textAttrs[NSForegroundColorAttributeName] = UIColor.blackColor;//tzImagePickerVc.barItemTextColor;
