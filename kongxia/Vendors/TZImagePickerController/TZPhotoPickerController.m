...
@interface TZPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate, TZAlbumViewDelegate> {
...
@property (nonatomic, strong) TZAlbumView *albumsView;
@property (nonatomic, strong) UIView *albumsBgView;
...
- (void)viewDidLoad {
    ...
    if (_isFromChat) {
        _titleView = [[TZPhotoPickerTitleView alloc] initWithFrame:CGRectMake(0.0, 0.0, 100.0, 31.0)];
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(showAlbums)];
...
- (void)configAlbumsView {
    _albumsBgView = [[UIView alloc] initWithFrame:self.view.bounds];
    _albumsBgView.backgroundColor = UIColor.blackColor;
    _albumsBgView.alpha = 0.0;
    
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(showAlbums)];
    [_albumsBgView addGestureRecognizer:tap];
    [self.view addSubview:_albumsBgView];
    
    _albumsView = [[TZAlbumView alloc] init];
    _albumsView.delegate = self;
    _albumsView.navigationController = (TZImagePickerController *)self.navigationController;
    _albumsView.backgroundColor = UIColor.brownColor;
    _albumsView.alpha = 0.0;
    [_albumsView fetchAlbums];
...
- (void)viewDidLayoutSubviews {
    ...
    _collectionView.frame = CGRectMake(0, 0, self.view.tz_width, collectionViewHeight);
    if (!_didShowAlbumsView) {
        _albumsView.frame = CGRectMake(0.0, _collectionView.top - 300.0, SCREEN_WIDTH, 300.0);
    }
...
- (void)showAlbums {
    ...
    if (_didShowAlbumsView) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            _albumsView.top = _collectionView.top - _albumsView.height;
            _albumsView.alpha = 0.0;
            _albumsBgView.alpha = 0.0;
        }];
    }
        ...
        [_albumsView show];
        [UIView animateWithDuration:0.3 animations:^{
            _albumsView.top += _albumsView.height;
            _albumsView.alpha = 1.0;
            
            _albumsBgView.alpha = 0.6;
        }];
    }
    _didShowAlbumsView = !_didShowAlbumsView;
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        ...
        if (isSelected) {
            ...
            // 2. select:check if over the maxImagesCount / 选择照片,检查是否超过了最大个数的限制 / 是否付费
            if (tzImagePickerVc.selectedModels.count < tzImagePickerVc.maxImagesCount) {
                if (_chatShouldPay) {
                    NSInteger currentTotalCost = [ZZUserHelper shareInstance].consumptionMebi;
                    NSInteger selectNeedToPay = [[ZZUserHelper shareInstance].configModel.priceConfig.per_chat_cost_mcoin integerValue] * (tzImagePickerVc.selectedModels.count + 1);
                    if ([[ZZUserHelper shareInstance].loginer.mcoin integerValue] < (currentTotalCost + selectNeedToPay)) {
                        
                        NSString *error = tzImagePickerVc.selectedModels.count == 0 ? @"么币余额不足，充值后再继续和TA私信" : @"您当前么币余额不足，请充值后再尝试勾选、发送更多消息";
                        [UIAlertController presentAlertControllerWithTitle:error message:nil doneTitle:@"立即充值" cancelTitle:@"取消" showViewController:self.navigationController completeBlock:^(BOOL isCancelled) {
                            if (!isCancelled) {
                ...
                if (_isFromChat) {
                    if (tzImagePickerVc.selectedModels.count > 0 && tzImagePickerVc.selectedModels.firstObject.asset.mediaType != model.asset.mediaType) {
                    ...
                    if (model.asset.mediaType == PHAssetMediaTypeVideo) {
                        ...
                        if (timeLength.integerValue > 60) {
                            [tzImagePickerVc showAlertWithTitle:@"不能分享超过1分钟的视频哦"];
                            return;
                        }
                    }
                }
...
#pragma mark - TZAlbumViewDelegate
- (void)view:(TZAlbumView *)view didSelectAlbum:(TZAlbumModel *)albumModel isTheFirstTime:(BOOL)isTheFirstTime{
    _model = albumModel;
    [_titleView configureAlbumName:albumModel.name];
    [self fetchAssetModels:NO];
    [self scrollCollectionViewToBottom];
    if (!isTheFirstTime) {
        [self showAlbums];
        return;
...
- (void)setModel:(TZAlbumModel *)model {
    _model = model;
    [_noDataLabel setHidden:model.count != 0];
}
...
- (void)configureAlbumName:(NSString *)albumName {
    _titleLabel.text = albumName;
    _iconImageView.image = [UIImage imageNamed:@"icDownAlbum"];
    
    CGFloat textWidth = [NSString findWidthForText:_titleLabel.text havingWidth:200 andFont:_titleLabel.font];
    
    
    CGFloat totalWidth = 12 + textWidth + 12.0 + 20.0 + 5.0;
    self.width = totalWidth;
    _titleLabel.width = textWidth;
    _iconImageView.left = _titleLabel.right + 12;
}

- (void)showUp {
    _iconImageView.image = [UIImage imageNamed:@"icUpAlbum"];
}
...
#pragma mark - Layout
- (void)layout {
    self.backgroundColor = RGBCOLOR(83, 83, 83);
    ...
    CGFloat textWidth = [NSString findWidthForText:_titleLabel.text havingWidth:200 andFont:_titleLabel.font];
    _titleLabel.frame = CGRectMake(12.0, 0.0, textWidth, self.height);
    _iconImageView.frame = CGRectMake(_titleLabel.right + 12.0, 5, 20, 20);
