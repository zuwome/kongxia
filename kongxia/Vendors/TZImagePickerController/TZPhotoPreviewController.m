...
- (void)select:(UIButton *)selectButton {
    ...
    if (!selectButton.isSelected) {
        ...
        if (_tzImagePickerVc.selectedModels.count >= _tzImagePickerVc.maxImagesCount) {
            ...
            if (_tzImagePickerVc.chatShouldPay) {
                NSInteger currentTotalCost = [ZZUserHelper shareInstance].consumptionMebi;
                NSInteger selectNeedToPay = [[ZZUserHelper shareInstance].configModel.priceConfig.per_chat_cost_mcoin integerValue] * (_tzImagePickerVc.selectedModels.count + 1);
                if ([[ZZUserHelper shareInstance].loginer.mcoin integerValue] < (currentTotalCost + selectNeedToPay)) {
                    
                    NSString *error = _tzImagePickerVc.selectedModels.count == 0 ? @"么币余额不足，充值后再继续和TA私信" : @"您当前么币余额不足，请充值后再尝试勾选、发送更多消息";
