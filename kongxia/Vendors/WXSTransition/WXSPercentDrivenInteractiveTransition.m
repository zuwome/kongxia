...
-(void)panAction:(UIPanGestureRecognizer *)pan{
    ...
    CGPoint translation = [pan translationInView:self.vc.view];
    WXSGestureType type = [self commitTranslation:translation];
    if (type != self.getstureType) {
        if (_isInter) {
            _isInter = NO;
            [self continueAction];
        }
...
- (WXSGestureType)commitTranslation:(CGPoint)translation
{
    CGFloat absX = fabs(translation.x);
    CGFloat absY = fabs(translation.y);
    if (absX >= absY ) {
        if (translation.x<0) {
            return WXSGestureTypePanLeft;
        }else{
            return WXSGestureTypePanRight;
        }
    } else  {
        if (translation.y<0) {
            return WXSGestureTypePanUp;
        }else{
            return WXSGestureTypePanDown;
        }
    }
}
...
- (void)UIChange {
    ...
    if (_animationType == WXSTransitionAnimationTypeCustomZoom) {
        if (_percent >= 0.5) {
