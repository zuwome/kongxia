...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo;
{
    ...
    label.bounds = CGRectMake(0, 0, expectSize.width, 40);
...
//    CGRect rect = [self.sourceImagesContainerView convertRect:self.currentView.frame toView:self];
    
    UIImageView *tempView = [[UIImageView alloc] init];
    tempView.frame = self.sourceRect;
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(photoBrowser:currentIndex:)]) {
        [_delegate photoBrowser:self currentIndex:self.currentImageIndex];
    }
}
...
//    NSLog(@"%@",NSStringFromCGRect(targetTemp));
    ...
    CGFloat tempImageViewH = 0.1f;
    if (tempImageSizeW) {
        tempImageViewH = (tempImageSizeH * kAPPWidth)/tempImageSizeW;
    }
    ...
    if (self.showAnimation) {
        [UIView animateWithDuration:HZPhotoBrowserHideImageAnimationDuration animations:^{
            tempImageView.frame = self.sourceRect;
        } completion:^(BOOL finished) {
