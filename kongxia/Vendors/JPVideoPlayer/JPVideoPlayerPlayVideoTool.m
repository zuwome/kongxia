...
- (nullable JPVideoPlayerPlayVideoToolItem *)playExistedVideoWithURL:(NSURL * _Nullable)url fullVideoCachePath:(NSString * _Nullable)fullVideoCachePath options:(JPVideoPlayerOptions)options showOnView:(UIView * _Nullable)showView playingProgress:(JPVideoPlayerPlayVideoToolPlayingProgressBlock _Nullable )progress error:(nullable JPVideoPlayerPlayVideoToolErrorBlock)error{
    ...
    }
    {
       ...
       item.timeObserver = [item.player addPeriodicTimeObserverForInterval:CMTimeMake(1.0, 1.0) queue:dispatch_get_main_queue() usingBlock:^(CMTime time){
...
- (void)appDidEnterPlayGround{
    // fixed #35.
    if (!_stopWhenAppDidEnterPlayGround) {
...
- (void)playerItemDidPlayToEnd:(NSNotification *)notification{
    ...
    if (self.currentPlayVideoItem.player.status == AVPlayerStatusReadyToPlay) {
    ...
    if ([keyPath isEqualToString:@"status"]) {
        ...
        switch (status) {
            ...
            case AVPlayerItemStatusReadyToPlay:{
                ...
                if (!self.currentPlayVideoItem||self.currentPlayVideoItem.player==nil) return;
...
//             NSLog(@"PY_当缓冲是空的时候_等会在播放");
//            [weakSelf hideActivaityIndicatorView];
//            [weakSelf.currentPlayVideoItem.player play];
//            if (!weakSelf.currentPlayVideoItem.currentPlayerItem.playbackLikelyToKeepUp) {
//                [weakSelf hideActivaityIndicatorView];
//                [weakSelf.currentPlayVideoItem.player play];
//            }
//        });
