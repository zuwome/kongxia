...
#define PLSImagesVIewW (SCREEN_WIDTH / PLSMinImageCount) // 图片宽度
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.scaledIamgeView.contentMode = UIViewContentModeScaleAspectFit;
    ...
    [self getImagesCount:imageCount imageBackBlock:^(UIImage *image) {
        if (image) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (imageCount - 1 == weakSelf.collectionImages.count) {
                    BLOCK_SAFE_CALLS(weakSelf.didThumbnailCompleted);
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        ...
        CGRect collectionRect = CGRectMake(0, 0, SCREEN_WIDTH, PLSImagesViewH);
