...
@interface ZZRentFansViewController () <UITableViewDataSource,UITableViewDelegate>
...
@implementation ZZRentFansViewController
...
- (void)pullWithSort_value:(NSString *)sort_value
{
    ...
    if ([ZZUserHelper shareInstance].isLogin) {
        [ZZUserFollowModel getUserFansListWithParam:aDict uid:self.user.uid next:^(ZZError *error, id data, NSURLSessionDataTask *task) {
            [self getData:data error:error sort_value:sort_value];
        }];
    } else {
        [ZZUserFollowModel getUnloginUserFansListWithParam:aDict uid:self.user.uid next:^(ZZError *error, id data, NSURLSessionDataTask *task) {
            [self getData:data error:error sort_value:sort_value];
        }];
    }
}
...
- (ZZFansEmptyView *)emptyView
{
    if (!_emptyView) {
        ...
        _emptyView.contentLabel.text = @"快来关注TA 成为TA第一个粉丝吧";
