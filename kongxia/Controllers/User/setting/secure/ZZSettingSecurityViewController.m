    ...
    [ZZRequest method:@"GET" path:@"/api/user/account/status" params:nil next:^(ZZError *error, id data, NSURLSessionDataTask *task) {
        if (error) {
            ...
            if (canCancelAccount) {
                ...
                [UIAlertView showWithTitle:@"邀约未完成" message:@"您还有正在进行中的邀约，暂时无法注销账号。" cancelButtonTitle:@"确定" otherButtonTitles:nil tapBlock:^(UIAlertView * _Nonnull alertView, NSInteger buttonIndex) {
