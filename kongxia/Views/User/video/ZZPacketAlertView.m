...
@implementation ZZPacketAlertView
...
- (void)showPriceInfoView
{
    ...
    if (self.priceBgView.hidden) {
        ...
        [_infoLabel mas_updateConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(_downBtn.mas_bottom).offset(height+15);
        }];
    } else {
        _downImgView.transform = CGAffineTransformMakeRotation(2* M_PI);
        self.priceBgView.hidden = YES;
        [_infoLabel mas_updateConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(_downBtn.mas_bottom).offset(15);
        }];
    }
}
...
- (UIView *)btnBgView
{
    if (!_btnBgView) {
        ...
        [_bgView sendSubviewToBack:lineView];
        ...
        NSArray *normalArray = @[@"icon_rent_share_timeline_n",@"icon_rent_share_wx_n",@"icon_rent_share_qq_n",@"icon_rent_share_wb_n"];
        NSArray *selectArray = @[@"icon_rent_share_timeline_p",@"icon_rent_share_wx_p",@"icon_rent_share_qq_p",@"icon_rent_share_wb_p"];
        CGFloat width = 35;
        CGFloat space = (SCREEN_WIDTH - 40 - 30 - 4*width)/3.0;
        for (int i=0; i<4; i++) {
            UIButton *typeBtn = [[UIButton alloc] init];
            [typeBtn addTarget:self action:@selector(typeBtnClick:) forControlEvents:UIControlEventTouchUpInside];
            [typeBtn setImage:[UIImage imageNamed:normalArray[i]] forState:UIControlStateNormal];
            [typeBtn setImage:[UIImage imageNamed:selectArray[i]] forState:UIControlStateSelected];
            typeBtn.tag = 100 + i;
            [_btnBgView addSubview:typeBtn];
            
            [typeBtn mas_makeConstraints:^(MASConstraintMaker *make) {
                make.left.mas_equalTo(_btnBgView.mas_left).offset(i*(width + space));
                make.top.mas_equalTo(_btnBgView.mas_top);
                make.bottom.mas_equalTo(_btnBgView.mas_bottom);
...
- (UIButton *)shareBtn
{
    if (!_shareBtn) {
        ...
        [_shareBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(_btnBgView.mas_bottom).offset(20);
            make.left.mas_equalTo(_bgView.mas_left).offset(15);
            make.right.mas_equalTo(_bgView.mas_right).offset(-15);
