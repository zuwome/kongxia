...
@interface ZZRequestLiveStreamAlert ()
...
@implementation ZZRequestLiveStreamAlert
...
//    }
    if (isPublish) {
        ...
        if (isNullString([ZZUserHelper shareInstance].configModel.skill.des)) {
            ...
            _contentLabel.text = [NSString stringWithFormat:@"视频咨询每%@分钟赠送%@张咨询卡，确认发布任务？",[ZZUserHelper shareInstance].configModel.priceConfig.settlement_unit,[ZZUserHelper shareInstance].configModel.priceConfig.per_unit_cost_card];
            
        }
        [_sureBtn setTitle:@"确认" forState:UIControlStateNormal];
    } else {
        _titleLabel.text = @"与TA视频";
        _contentLabel.text = [NSString stringWithFormat:@"视频咨询每%@分钟赠送%@张咨询卡，确认发布任务？",[ZZUserHelper shareInstance].configModel.priceConfig.settlement_unit,[ZZUserHelper shareInstance].configModel.priceConfig.per_unit_cost_card];

        [_sureBtn setTitle:@"确认拨打" forState:UIControlStateNormal];
    }
}
...
- (void)sureBtnClick
{
    ...
    if (_selected) {
        if (_isPublish) {
            [ZZKeyValueStore saveValue:@"firstPublishOrderAlert" key:[ZZStoreKey sharedInstance].firstPublishOrderAlert];
        } else {
            [ZZKeyValueStore saveValue:@"firstConnectAlert" key:[ZZStoreKey sharedInstance].firstConnectAlert];
        }
    }
}
...
- (UIView *)bgView
{
    if (!_bgView) {
        ...
        self.titleLabel.text = @"发起1V1视频";
        self.contentLabel.text = @"视频咨询每2分钟赠送4张咨询卡，确认发布任务";
