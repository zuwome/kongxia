...
@interface ZZOnlineConfirmAlert ()
...
@implementation ZZOnlineConfirmAlert
...
- (UIView *)bgView {
    if (!_bgView) {
        ...
        self.titleLabel.text = @"1V1视频通话";
        self.centerImgView.image = [UIImage imageNamed:@"bgPopupVedio"];
        self.ignoreBtn.hidden = NO;

        NSString *string = @"* 请耐心等待对方选择\n* 仪容仪表端庄 大方出镜\n* 视频结束后获得收入";
        NSMutableAttributedString *hintString = [[NSMutableAttributedString alloc] initWithString:string];
        NSRange range1 = [string rangeOfString:@"* 请"];
        NSRange range2 = [string rangeOfString:@"* 仪"];
        NSRange range3 = [string rangeOfString:@"* 视"];
        ...
        [hintString addAttribute:NSForegroundColorAttributeName value:RGBCOLOR(244, 203, 7) range:NSMakeRange(range3.location, 1)];
...
- (void)sureBtnClick
{
    ...
    if (_selected) {
        [ZZKeyValueStore saveValue:@"firstOnline" key:[ZZStoreKey sharedInstance].firstOnline];
    }
}
