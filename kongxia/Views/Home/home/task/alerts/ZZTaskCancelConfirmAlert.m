...
@interface ZZTaskCancelConfirmAlert ()
...
@implementation ZZTaskCancelConfirmAlert
...
- (void)rightBtnClick
{
    ...
    if (_selected) {
        [ZZKeyValueStore saveValue:@"firstTaskCancelConfirmAlert" key:[ZZStoreKey sharedInstance].firstTaskCancelConfirmAlert];
    }
}
...
- (UIView *)bgView
{
    if (!_bgView) {
        ...
        imgView.image = [UIImage imageNamed:@"icon_task_cancel_alert"];
        [_bgView addSubview:imgView];
        
        [imgView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerX.mas_equalTo(_bgView.mas_centerX);
            make.top.mas_equalTo(_bgView.mas_top).offset(30);
            make.size.mas_equalTo(CGSizeMake(276.5*scale, 147.5*scale));
        }];
        ...
        [leftBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.top.mas_equalTo(_ignoreBtn.mas_bottom).offset(15);
