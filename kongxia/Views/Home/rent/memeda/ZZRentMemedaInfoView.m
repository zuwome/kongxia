...
@implementation ZZRentMemedaInfoView
{
    UIView                  *_vBgView;
}
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [_levelImgView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(nameBgView.mas_right).offset(8);
        ...
        [_vLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(_vBgView.mas_top).offset(2);
            make.left.mas_equalTo(_vBgView.mas_left);
            make.right.mas_equalTo(_vBgView.mas_right);
            make.bottom.mas_equalTo(_vBgView.mas_bottom).offset(-2);
        }];
...
- (void)setData:(ZZUser *)user
{
    ...
    [_headImgView sd_setImageWithURL:[NSURL URLWithString:user.avatar] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
