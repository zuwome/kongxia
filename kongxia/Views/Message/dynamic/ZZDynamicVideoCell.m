...
@implementation ZZDynamicVideoCell
{
    CGFloat             _width;
}
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        _width = SCREEN_WIDTH - 10 - 38 - 18 - 2;
        [self createViews];
    }
...
- (void)setData:(ZZMessageAttentDynamicModel *)model
{
    ...
    if ([model.type isEqualToString:@"sk"]) {
        ...
        _videoContentLabel.text = skModel.content;
        [_coverImgView sd_setImageWithURL:[NSURL URLWithString:skModel.video.cover_url]];
        _typeImgView.image = [UIImage imageNamed:@"icon_dynamic_sk"];
        _videoTimeView.typeImgView.image = [UIImage imageNamed:@"icon_video_sk"];
        _videoTimeView.timeLabel.text = [NSString stringWithFormat:@"%.f秒",skModel.video.time];
        self.readCountView.countLabel.text = [ZZUtils getCountStringWithCount:skModel.browser_count];
        self.commentCountView.countLabel.text = [ZZUtils getCountStringWithCount:skModel.reply_count];
        self.zanCountView.countLabel.text = [ZZUtils getCountStringWithCount:skModel.like_count];
    } else {
        ZZMMDModel *mmdModel = model.mmds[0];
        _videoContentLabel.text = mmdModel.content;
        ZZMMDAnswersModel *answerModel = mmdModel.answers[0];
        ...
        self.readCountView.countLabel.text = [ZZUtils getCountStringWithCount:mmdModel.browser_count];
        self.commentCountView.countLabel.text = [ZZUtils getCountStringWithCount:mmdModel.reply_count];
        self.zanCountView.countLabel.text = [ZZUtils getCountStringWithCount:mmdModel.like_count];
    }
