...
@implementation ZZChatBurnAttentView
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        topImgView.image = [UIImage imageNamed:@"icon_chat_burn_attent"];
        [bgView addSubview:topImgView];
        
        [topImgView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerX.mas_equalTo(bgView.mas_centerX);
            make.top.mas_equalTo(bgView.mas_top).offset(30);
            make.size.mas_equalTo(CGSizeMake(126.5*scale, 95.5*scale));
        }];
