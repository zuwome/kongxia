...
class LiveStreamRechargeAlertView: UIView {
    @objc var rechargeClousure : StartVideoChatBlock?
...
extension LiveStreamRechargeAlertView {
    @objc func setupData(content: String, subContent: String) {
        titleLabel.text = "发起1V1视频"
        iconImageView.image = UIImage(named: "icon_livestream_video")
        contentLabel.text = content
        infoLabel.text = subContent
    }
}
extension LiveStreamRechargeAlertView {
    func layoutUI() {
        ...
        contentView.mas_makeConstraints { make in
            make?.center.equalTo()(self)
            make?.size.mas_equalTo()(CGSize(width: width, height: 260))
        }
