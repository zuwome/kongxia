...
@implementation ZZChatWechatPayCell
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        _iconImageView.image = [UIImage imageNamed:@"wechatSCopy2"];
        ...
        [_iconImageView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.size.mas_equalTo(CGSizeMake(21, 17));
        }];
        ...
        [_titleCustomLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(_iconImageView.mas_right).offset(7);
            make.top.mas_equalTo(_bgView.mas_top).offset(10);
        }];
...
- (void)setData:(ZZChatBaseModel *)model {
    _dataModel = model;
    ZZMessageChatWechatPayModel *infoModel = (ZZMessageChatWechatPayModel *)_dataModel.message.content;
    if (UserHelper.configModel.wechat_new && !isNullString(infoModel.wechatseenId)) {
        ...
        [_titleCustomLabel mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(_iconImageView.mas_right).offset(7);
    ...
    if ([model.message.content isKindOfClass:[ZZMessageChatWechatPayModel class]]) {
        ...
        NSLog(@"sssssss:%@",infoModel.wechatseenId);
        if ([_evaluationType isEqualToString:@"1"]) {
...
- (void)weiChatPayClick     {
    if (UserHelper.configModel.wechat_new) {
        ZZMessageChatWechatPayModel *infoModel = (ZZMessageChatWechatPayModel *)_dataModel.message.content;
        if (!isNullString(infoModel.wechatseenId)) {
            [[self nextResponder] routerEventWithName:ZZWeiChatCheckOrder
                                             userInfo:@{@"wechatOrder":infoModel.wechatseenId} Cell:self];
            return;
        }
    }
    else {
...
//        else if ([_evaluationType isEqualToString:@"2"]) {
//            [[self nextResponder] routerEventWithName:ZZShowUpdateView
//                                             userInfo:nil
//                                                 Cell:self];
//        }
    }
